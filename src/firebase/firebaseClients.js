...
export class AuthHandler {
  ...
  async registerUser(email, password) {
    try {
      this.userCred = await createUserWithEmailAndPassword(
        this.appAuth,
        email,
        password
      );
  ...
  async signIn(email, password) {
    try {
      this.userCred = await signInWithEmailAndPassword(
        this.appAuth,
        email,
        password
      );
...
export class DatabaseHandler {
  ...
  async getReadingList(userId) {
    ...
    const docs = await getDocs(
      collection(this.database, "users", userId, "readinglist")
    );
    const fileMap = {};
    docs.forEach((el) => {
  ...
  async addToReadingList(listItemObj, userId, nonce) {
    ...
    const docInfo = await setDoc(
      doc(this.database, "users", userId, "readinglist", nonce),
      listItemObj
    );
  ...
  async getJargon(userId) {
    ...
    const docs = await getDocs(
      collection(this.database, "users", userId, "jargonlist")
    );
    const fileMap = {};
    docs.forEach((el) => {
      fileMap[el.id] = el.data();
  ...
  async addToJargonList(listItemObj, userId, nonce) {
    ...
    const docInfo = await setDoc(
      doc(this.database, "users", userId, "jargonlist", nonce),
      listItemObj
    );
  ...
  async getSettings(userId) {
    ...
    const docs = await getDocs(
      collection(this.database, "users", userId, "settingsList")
    );
    const fileMap = {};
    docs.forEach((el) => {
      fileMap[el.id] = el.data();
  ...
  async changeSetting(userId, nonce, listItemObj) {
    ...
    const docInfo = await setDoc(
      doc(this.database, "users", userId, "settingsList", nonce),
      listItemObj
    );
