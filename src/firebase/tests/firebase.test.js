...
describe('Test Authentication: ', () => {
  test('Checks if signed in user is authenticated', () => {
    return auth.signIn(testEmail, testPassword)
      .then((usr) => {
...
describe('Test Each Part of Database: ', () => {
  test('Pulls reading list and confirms correctness', () => {
    return auth.signIn(testEmail, testPassword)
    .then((usr) => {
      return db.getReadingList(getAuth().currentUser.uid)
      .then((rlData) => {
  ...
  test('Pulls jargon list and confirms correctness', () => {
    return auth.signIn(testEmail, testPassword)
    .then((usr) => {
      db.getJargon(getAuth().currentUser.uid)
      .then((jargonData) => {
  ...
  test('Pulls settings list and confirms correctness', () => {
    return auth.signIn(testEmail, testPassword)
    .then((usr) => {
      db.getSettings(getAuth().currentUser.uid)
      .then((settingData) => {
